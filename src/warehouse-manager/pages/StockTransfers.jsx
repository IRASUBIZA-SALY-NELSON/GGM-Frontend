...
const StockTransfers = () => {
  ...
  const fetchTransfers = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8001/api/stock-transfers', {
  ...
  const updateTransferStatus = async (transferId, newStatus) => {
    try {
      const response = await fetch(`http://localhost:8001/api/stock-transfers/${transferId}/status`, {
        method: 'PUT',
        headers: getAuthHeaders(),
